...
public class ImageUtil {
	...
	private static boolean isPic(String sourceImagePath) {
		...
		if (sourceImagePath.indexOf(".") > 0) {
			String fileSfix = sourceImagePath.substring(sourceImagePath.indexOf("."), sourceImagePath.length())
					.replace(".", "");
	...
	public static void main(String[] args) throws IOException {
		...
		// File[] subFile = file.listFiles();
		// for (int i = 0; i < subFile.length; i++) {
		// ImageUtil.createImage(subFile[i].getAbsolutePath(),
		// ImageUtil.resize(subFile[i].getAbsolutePath(), 150, 100));
		// }
	}
	 ...
	 */
	public static boolean resizeImageForEncode(File sourceImageFile, String savePath, Integer width, Integer height,
			String sufix) {
		try {
...
//			ImageOutputStream encoder = ImageIO.createImageOutputStream(newimage);
...
//	}
	 ...
	 *            是否多张图片
	 * @param exp
	 *            多张图片的规则，例如1.jpg|2.jpg|3.jpg 那么exp=|
	 */
	public static void zoom(String sourceImagePath, String savePath, int targetWidth, int targetHeight, boolean more,
			String exp) {
		File file = null;
		BufferedImage original = null;
		BufferedImage bufOut = null;
