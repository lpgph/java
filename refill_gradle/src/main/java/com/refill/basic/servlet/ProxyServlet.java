...
import com.refill.util.proxy.Header;
...
public class ProxyServlet extends BaseServlet {
	 ...
	 *            HttpServletResponse 对象
	 * @param way
	 *            请求方式,true:发送GET请求,false:发送POST请求
	 * @throws IOException
	 *             IO异常处理
	 */
	private void proxy(HttpServletRequest request, HttpServletResponse response, Boolean way) throws IOException {
		...
		try {
			...
			// 请求头部信息
			Header header = new Header(hostUrl);
			...
			Result result = way ? Proxy.get(url, header, paramsMap, charset)
